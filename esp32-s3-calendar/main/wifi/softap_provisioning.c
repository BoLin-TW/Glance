...
#define PROV_MDNS_HOSTNAME         "esp-setup-wifi"
...
static void start_mdns_service(void) {
    ...
    ESP_ERROR_CHECK(mdns_hostname_set(PROV_MDNS_HOSTNAME));
    ESP_LOGI(TAG, "mDNS hostname set to: [%s]", PROV_MDNS_HOSTNAME);
    ESP_ERROR_CHECK(mdns_instance_name_set("ESP32 Wi-Fi Setup"));
}
...
static void softap_prov_deinit_wifi(void) {
    ...
    stop_mdns_service(); // Stop mDNS service
